...
+------------------------------------------------+----------+---------+-----------------------+---------------+------------------------------------------------+---------+---------------------------------------+
| {system_name} |  admin   |   True  | connection successful |     {system_serial}     | {system_name} |   {pool_id}  | {pool_name} |
+------------------------------------------------+----------+---------+-----------------------+---------------+------------------------------------------------+---------+---------------------------------------+
"""
...
class CommandlineTestsMixin(object):
    ...
    def test_set_protocol(self):
        with self.provisioning_pool_context() as pool:
            ...
            stderr = 'done, please restart cinder-volume service for changes to take effect\n'
            pid = self.assert_command(args, stderr=stderr)
            args = ["volume-backend", "set-protocol", "fc", self.infinisdk.get_name(), str(pool.get_id()), "--commit"]
            pid = self.assert_command(args, stderr=stderr)
...
class RealInfiniBoxMixin(InfiniboxMixin):
    POOL_CREATE_KWARGS = dict(physical_capacity=30*GiB, virtual_capacity=30*GiB)
...
class MockInfiniBoxMixin(InfiniboxMixin):
    POOL_CREATE_KWARGS = dict()
